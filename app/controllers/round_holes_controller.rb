class RoundHolesController < ApplicationController
  ...
  def show
  #needs to go to the next hole here, but only till the 18th or end
  #@round_hole =  
  redirect_to @round_hole.round 
  end
  ...
  private
    ...
    def round_hole_params
      params.require(:round_hole).permit(:round_id, :course_id, :score, :strokes, :putts, :notes, :round, :course_hole_id)
    end
end
