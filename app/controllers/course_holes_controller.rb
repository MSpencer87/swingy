class CourseHolesController < ApplicationController
  ...
  def new
    @course = Course.find(params[:course_id])
    @course_hole = @course.course_holes.new
  end
  ...
  def create
    @course = Course.find(params[:course_id])
    @course_hole = @course.course_holes.new(course_hole_params)
    @course_hole.course = @course.name

    respond_to do |format|
      if @course_hole.save
        format.html { redirect_to course_course_holes_url, notice: 'Course hole was successfully created.' }
  ...
  private
    ...
    def course_hole_params
      params.require(:course_hole).permit(:course_id, :number, :par, :distance, :rating, :notes, :course_id)
    end
end
